...
docker exec \
    -w /mnt \
    -t bonsai \
    bash -c "source /cellstates/cellstates_venv/bin/activate && mkdir -p cellstates_results && python /cellstates/scripts/run_cellstates.py $CELLSTATES_ARGS  -o cellstates_results"

echo "Cellstates run finished."
